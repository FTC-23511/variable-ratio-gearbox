...
## 4. Tier rules — tailored for this project

This is a **single-file `index.html` app** (no backend, no DB, no CI yet), so
tiers are classified by the *nature* of the change, not by path. "Green" = the
build brief §6 acceptance self-tests still pass when `index.html` is opened in a
browser.
...
- Documentation: `docs/`, root-level `*.md` (`README.md`, `CLAUDE.md`), the brief
  and spec files.
- UI / styling / layout tweaks in `index.html` that don't change the physics.
- Adding or adjusting config inputs/sliders within the ranges in build brief §5.
- New plot/trace rendering, loss-budget display, formatting/refactor with no
  change to the solve math or acceptance-test results.

### Approval-required tier — STOP and ask

Do **not** auto-merge. Post a plain-English summary, then leave the PR open, when
the change touches the simulation's correctness or the project's invariants:

- The **per-timestep solve order** or any equation in spec §3 (steps A–J).
- The **hard-coded goBILDA 5000-series motor constants** (`R_m`, `Kt`, `Ke`,
  `I_free`) — these are fixed, not user-editable (brief §5, spec §2).
- The **control policies** in spec §4 (accel/cruise/brake, gear selection, CVT
  grid search) or the outer ratio optimizer (spec §5).
- Removing or loosening any **clamp** (current, brownout `I_bn`, traction
  `F_tract_max`, segment timeout guard) or a division guard.
- Changing **acceptance-test thresholds** in brief §6, or any change that makes a
  self-test fail.
- Introducing a build step, framework, server, backend, or second file — the
  single-file constraint is a hard scope rule (brief §1, §7).
- Any change you're not certain is reversible, or any deletion of more than ~50
  lines from `index.html`.

When you stop for approval, the PR comment states: what it changes (plain
English), what it touches (physics / motor constants / policy / scope), and
